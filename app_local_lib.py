...
def app_local_lib():
    ...
    def load_csv():
        try:
            df = pd.read_csv(r"C:\Users\406\Documents\GitHub\books_for_me\data\지역코드.csv")  # CSV 경로 수정 필요
            return df
    ...
    df = load_csv()
        ...
        # 인증키 입력
    AUTH_KEY = "f6727fccbbc455cac9b14af79ef28e4b3b9d8db898c217c7abb19bfde8b0d8d8"
    ...
    # === 저장 및 CSV 매핑 ===
    if st.button("지역 선택", type="primary"):
                        ...
                        try:
                            ...
                            if "response" in data and "items" in data["response"]:
                                ...
                                if libs:
                                    ...
                                    st.success(f" {region} {dtl_region} 에서 {len(libs)}개 도서관에 소장 중입니다.")

                                    # 도서관 목록 표 출력
                                    lib_df = pd.DataFrame(libs)
                                    st.dataframe(lib_df[['libCode', 'libName', 'address', 'tel']], use_container_width=True)
                                else:
                                    st.warning(f" {region} {dtl_region}에는 소장 도서관이 없습니다.")
                            else:
